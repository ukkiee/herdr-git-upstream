...
**fetch는 좁게 한다.** 현재 브랜치의 참조 하나만 가져오고, 태그와 prune과 하위 모듈은 건드리지
않으며 자동 정리(`gc.auto`)도 꺼 둔다. 주기적으로 도는 작업이 사용자 모르게 무거운 일을 반복하지
않게 하려는 것이다. 원격 이름은 `origin`으로 단정하지 않고 `branch.<이름>.remote`에서 읽는다.
