...
로컬에서 `git pull`, 커밋, 브랜치 전환을 하면 다음 fetch를 기다리지 않고 사이드바 토큰을 갱신합니다.
1초마다 Git 참조 파일의 변경 정보만 확인하며, 바뀐 저장소·체크아웃만 다시 계산합니다.
변경이 없으면 Git 명령이나 네트워크 요청을 실행하지 않습니다.
...
1. 제안된 브랜치 이름을 수정하세요. `←` / `→`로 커서를 옮겨 입력하고, `Backspace`로 커서 앞 글자를 지웁니다.
2. `Tab`으로 `Base`에 이동하고 `Enter`로 드롭다운을 여세요.
3. 브랜치 이름 일부를 입력해 검색하고, `↑` / `↓`로 결과를 골라 `Enter`로 확정하세요.
...
검색은 대소문자를 구분하지 않으며 결과 개수를 표시합니다. `Backspace`로 검색어를 고치고, 목록을 닫으면 검색어가 지워집니다.
검색창은 작은 선택 목록 상단에 있으며, 후보는 한 번에 최대 4개를 보여 줍니다. 화살표로 나머지 결과를
둘러볼 수 있고, 현재 강조한 브랜치의 종류와 fetch 상태는 목록 아래에 표시합니다.
검색어와 일치하는 글자는 선택 행에서도 굵은 청록색으로 강조합니다.
...
경고는 여러 줄로 표시합니다. `F1`로 전체 메시지를 열고 `↑` / `↓`로 스크롤하세요.
`Enter` 또는 `Esc`를 누르면 입력하던 화면으로 돌아옵니다.
