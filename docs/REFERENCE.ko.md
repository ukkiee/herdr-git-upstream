...
[시작 안내](../README.ko.md) · [English](REFERENCE.md) | **한국어**

설치와 기본 사용법은 README부터 읽는다. 이 문서는 토큰의 의미, 상세 설정, 화면 동작과 문제 해결을
다룬다. `./bin/`으로 시작하는 명령은 플러그인 저장소에서 실행한다.
`herdr plugin list --plugin git-upstream --json`의 `plugin_root`에서 그 경로를 확인한다.

[토큰](#토큰) · [설정](#설정) · [worktree 화면](#worktree-화면) ·
[생성 팝업](#생성-팝업) · [문제 해결](#문제를-살펴볼-때)

## 로컬 개발

Linux·macOS에서는 저장소 뿌리에서 실행한다.
...
`plugin link`는 실행 파일을 빌드하지 않는다. Go 코드를 바꿨으면 다시 빌드한다. Windows에서는
`powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build.ps1`로 빌드한 뒤 저장소 경로를 link한다.
...
`config.toml`에 빠진 사이드바 토큰과 worktree 화면 키 설정을 출력한다. 플러그인 설정에서 토큰 이름을
바꿨다면 바꾼 이름을 사용한다. 파일은 수정하지 않는다. 기존 테이블에 출력을 합친 뒤
...
위 주기는 정기 fetch 회차의 간격이다. 별도로 성공한 회차에서 발견한 워크스페이스의 Git 참조 변경
정보를 1초마다 확인한다. 공유 참조는 저장소당 한 번 읽고, 연결된 worktree를 포함해 각 체크아웃의
HEAD도 확인한다. 작업 파일과 객체 내용은 훑지 않는다. 변경이 없으면 Git 명령·fetch·토큰 보고를
실행하지 않는다. 변경된 저장소·체크아웃만 토큰을 다시 계산하며 보고 실패는 다음 검사에서 재시도한다.
보고 직전에 HEAD를 다시 읽고 보고를 직렬화하므로 늦게 끝난 fetch가 pull 이전 HEAD로 덮어쓰지 않는다.
새로 열거나 이동한 워크스페이스는 기존 포커스·정기 회차에서 발견한다. `gone`·`stale`은 플러그인의
fetch 기록을 계속 사용한다. 로컬 참조 변경만으로 원격 통신 성공을 추정하지 않는다.
변경 감지는 fetch와 독립적으로 돌지만 Git 계산과 herdr 보고 시간은 추가로 필요하다.
...
[ADR 0002](adr/0002-removal-boundary.md)에 있다.
...
`Base`는 접힌 선택 필드다. `Tab`으로 이동하고 `Enter`로 드롭다운을 연 뒤, 브랜치 이름 일부를 입력해
검색한다. 대소문자를 구분하지 않으며 검색 결과 안의 위치·결과 개수·전체 후보 개수를 표시한다.
`Backspace`로 검색어를 고치고, 목록을 닫으면 검색어가 지워진다. 결과가 없으면 선택할 수 없다.
접힌 필드는 이름 길이에 맞춘다. 목록은 최대 38칸 폭으로 후보를 최대 4개 보여 주며, 위쪽 테두리에
검색창, 아래쪽 테두리에 개수를 표시한다. 강조한 후보의 종류와 fetch 상태는 목록 아래에 표시한다.
긴 후보 이름은 중간을 말줄임표로 줄여 앞부분과 끝부분을 남긴다.
검색어와 일치하는 글자는 원래 대소문자를 유지한 채 굵은 청록색으로 강조하며, 선택 행 표시도
유지한다. 말줄임 전 이름에서 일치 범위를 찾고 화면에 남은 부분만 강조한다. 삽입한 말줄임표에는
색을 넣지 않으며 검색어를 지우면 강조도 사라진다.
`↑`/`↓`와 `Enter`로 기준을 확정한다. `Tab`으로 이름 칸에 돌아와 `Enter`를 누르면 생성한다. 후보를 둘러보는 동안에는
...
커서는 자동 이름 끝에 놓인다. `←`/`→`로 이동하며, 입력하면 커서 위치에 삽입하고 `Backspace`는
커서 바로 앞 글자를 지운다. 긴 이름은 커서가 보이도록 표시한다. 이름 끝에서는 추가 공백 없이
닫는 괄호로 삽입 위치를 표시하며, `Base`로 포커스를 옮기면 이름 커서는 숨긴다. 이름을 직접 고친 뒤에는
기준을 바꿔도 입력을 덮어쓰지 않는다.

경고는 팝업 폭에 맞춰 줄을 나눈다. `F1`은 입력 화면에 다 담기지 않는 부분까지 전체 메시지를 연다.
`↑`/`↓`로 스크롤하고 `Enter`, `Esc`, `F1`로 입력 화면에 돌아온다. 이때 생성하지 않으며 입력도 보존한다.
이전 생성이 완료되지 않았다는 경고는 생성 보호 기록이 pending인 경우다. 기존 브랜치 이름이라는
이유만으로 이 경고를 내지는 않는다.
...
| `↑` `↓` | 열린 기준 드롭다운에서 강조 이동 |
| `←` `→` | 브랜치 이름 칸에서 커서 이동 |
| 문자·`Backspace` | 이름 입력·수정. 기준 드롭다운이 열렸으면 검색어 입력·수정 |
| `Enter` | 이름 칸: 생성. 기준 필드: 드롭다운 열기. 열린 드롭다운: 기준 확정 |
| `Esc` | 드롭다운이 열렸으면 먼저 닫기, 닫혔으면 팝업 취소 |
| `Ctrl-C` | 팝업 취소 |
| `F1` | 전체 메시지 열기. 메시지 화면에서는 입력 화면으로 돌아가기 |
...
**포커스 훅은 fetch를 하지 않는다.** herdr는 동시에 도는 플러그인 명령을 32개로 제한한다. 포커스는
자주 바뀌고 fetch는 느리다. 훅이 그 자리를 수십 초씩 차지하면 탭 이름을 바꾸는 것 같은 다른
플러그인의 훅까지 밀린다. 그래서 훅은 파일 하나를 쓰고 곧바로 끝나며, 실제 작업은 herdr 바깥의
데몬이 맡는다. 드물게 발생하는 `worktree.created` 훅은 그 자리에서 fetch와 최신화를 실행한다.
사용자가 작업을 시작한 뒤에 기준이 뒤늦게 움직이는 일을 피하기 위해서다.
...
잡는다. 근거는 [ADR 0001](adr/0001-merged-judgement.md)에 있다.
