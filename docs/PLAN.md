...
**무엇을.** `herdr-git-upstream worktrees` 명령과 그것을 여는 팝업 하나.
...
**설정 없이 보이게 할 방법.** 사이드바 `rows` 에 토큰을 넣기 전에는 아무것도 보이지 않는다.
조사한 플러그인들이 하나같이 이 문제를 경고문으로 달아 두고 있었다. 세 가지를 생각해 두었다.

- 데몬이 시작할 때 사용자의 `config.toml` 을 읽어 우리 토큰이 없으면 herdr 알림으로 한 번 알린다
- `setup` 명령이 사용자의 현재 `rows` 에 우리 토큰을 끼워 넣은 결과를 출력한다.
  파일을 말없이 고치지 않는다
- 토큰을 넣지 않아도 fetch 는 돌고, 그것만으로 herdr 내장 `git_status` 가 정확해진다는 사실을
  README 첫머리에 적는다
